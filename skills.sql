-- 1. Select all columns form Brand table
    SELECT * FROM Brands;
...
-- 7. Select the name of any and all car brands that are not discontinued.
    SELECT name FROM Models WHERE discontinued IS NULL;
-- 8. Select rows 15-25 of the DB in alphabetical order by model name.
    SELECT * FROM Models ORDER BY name LIMIT 11 OFFSET 14;

-- 9. Select the brand, name, and year the mo.schedel's brand was 
--    founded for all of the models from 1960. Includ.schema
--    for model(s) even if its brand is not in the Brands table.
--    (The year the brand was founded should be NULL if 
--    the brand is not in the Brands table.)
    SELECT brand_name, b.name, founded 
    FROM Models AS m 
...
-- left joins return all the rows from left table eventhough there is not match in right table
-- inner joins return all the rows from both table as long as they match.
...
-- before: 
    ...
    FROM Brands AS b 
    LEFT JOIN Models ON (b.name = Models.brand_name) 
...
-- 1. Select the name of any brand with more than 5 models in the database.
    SELECT brand_name 
    FROM Models WHERE (SELECT COUNT(*) FROM Models)>5 
    GROUP BY brand_name;
...
VALUES ('IIHS Safety Award',2015, (SELECT id FROM Models WHERE year=2015 AND name = 'Subaru Outback'));
